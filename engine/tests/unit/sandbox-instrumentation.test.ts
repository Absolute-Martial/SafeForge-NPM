...
test("INSTRUMENTATION_JS patches network, process, env, eval, crypto, and timers", () => {
  ...
  assert.match(INSTRUMENTATION_JS, /tcp:\/\//);
  assert.match(INSTRUMENTATION_JS, /dns:\/\//);
  ...
  assert.match(INSTRUMENTATION_JS, /global\.Function/);
...
test("INSTRUMENTATION_JS flushes a serialized trace on process exit", () => {
  assert.match(INSTRUMENTATION_JS, /__SAFEFORGE_NPM_TRACE__/);
  assert.match(INSTRUMENTATION_JS, /__SAFEFORGE_NPM_TRACE_END__/);
