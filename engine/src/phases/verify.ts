...
function isValidJs(code: string): boolean {
  const tmpFile = join(tmpdir(), `safeforge-npm-syntax-check-${Date.now()}.js`);
  try {
...
Output ONLY the fixed JavaScript test code.`;
  ...
  try {
    ...
    const result = await generateText({
      model: getModel(config.testGenModel, llmRuntime),
...
export async function verifyProofs(
  ...
  const workDir = createRuntimeTempDir("safeforge-npm-verify-");
  ...
  const containerName = `safeforge-npm-verify-${randomUUID().slice(0, 12)}`;
  ...
  try {
    ...
    const verifyImage = "safeforge-npm-verify";
