...
export function extractLifecycleScripts(packageJson: Record<string, unknown>): Array<{ name: string; entry: string }> {
  ...
  return LIFECYCLE_HOOKS
    ...
    .map((hook) => ({
      name: `lifecycle:${hook}`,
      entry: (scripts as Record<string, string>)[hook]!,
    }));
}
...
function parseTraceFromOutput(stdout: string, stderr: string): ParsedTraceOutput {
  ...
  if (startIndex !== -1 && endIndex > startIndex) {
    ...
    try {
      trace = normalizeInstrumentationEvents(JSON.parse(json));
    } catch {
      trace = emptyTrace();
    }
  }
...
function normalizeInstrumentationEvents(raw: unknown): InstrumentationLog {
  if (!Array.isArray(raw)) {
    return InstrumentationLog.parse(raw);
  }
  ...
  for (const event of raw) {
    ...
    if (type === "require") {
      trace.modulesLoaded.push(String(record.module ?? ""));
      continue;
    }

    if (type === "network") {
      trace.networkCalls.push({
        method: String(record.method ?? "GET"),
        url: String(record.url ?? ""),
        bodyPreview: String(record.bodyPreview ?? ""),
      });
      continue;
    }

    if (type === "fs") {
      trace.fsOperations.push({
        op: String(record.method ?? "unknown"),
    ...
    if (type === "env") {
      trace.envAccess.push(String(record.key ?? ""));
      continue;
    }

    if (type === "process") {
      trace.processSpawns.push({
        cmd: String(record.cmd ?? record.method ?? ""),
    ...
    if (type === "eval") {
      trace.evalCalls.push({ code: String(record.code ?? "") });
      continue;
    }

    if (type === "crypto") {
      trace.cryptoOps.push({
  ...
  return InstrumentationLog.parse(trace);
}
...
function createPackageTarball(packagePath: string, workDir: string): string {
  ...
  const stableTarballPath = join(workDir, "package.tgz");
...
async function prepareCliProject(containerName: string): Promise<string | null> {
  const install = await dockerExec([
    ...
    [
      ...
      // Dependencies may need registry access during install, but package lifecycle
      // scripts must never run before we disconnect networking and enable tracing.
      "npm install --ignore-scripts --cache /workspace/.npm-cache /workspace/package.tgz",
    ].join(" && "),
  ], 120_000);
...
async function runLifecycleScript(
  containerName: string,
  packageName: string,
  script: CliCommandDescriptor,
  nodeVersion: string,
): Promise<CliCommandResult> {
  const scriptName = script.name.replace(/^lifecycle:/, "");
  const packageDir = `./node_modules/${packageName}`;
  const commandLine = `cd ${shellEscape(packageDir)} && ${script.entry}`;
  const started = Date.now();
  const exec = await dockerExec([
    "exec",
    "-e", "NODE_OPTIONS=--require /workspace/safeforge-instrumentation.js",
...
export async function runCliBehavior(
  ...
  const packageJson = JSON.parse(readFileSync(join(packagePath, "package.json"), "utf-8")) as Record<string, unknown>;
  const packageName = typeof packageJson.name === "string" ? packageJson.name : basename(packagePath);
  const cliCommands = await runner.discoverCommands(packagePath, packageName);
  const lifecycleScripts = extractLifecycleScripts(packageJson);
  const commands = [...lifecycleScripts, ...cliCommands];
  ...
  for (const nodeVersion of sandboxOptions.nodeVersions) {
    ...
    try {
      ...
      for (const script of lifecycleScripts) {
        const result = await runLifecycleScript(containerName, packageName, script, nodeVersion);
        allResults.push(result);
        emitCliResult(emit, result);
      }
