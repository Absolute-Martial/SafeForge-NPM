...
export async function runTriage(
  ...
  const fileVerdicts = await Promise.all(
    sourceFiles.map(async (f) => {
      try {
        const verdict = await analyzeFile(packagePath, f.path, flagsByFile.get(f.path) ?? [], llmRuntime, emit);
        completed++;
  ...
  const triageResult = await synthesizeTriageResult(inventory, fileVerdicts, llmRuntime);
