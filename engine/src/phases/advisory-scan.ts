...
const MAX_GHSA_ENRICHMENTS = 20;
const MAX_NVD_ENRICHMENTS = 8;
...
export async function scanAdvisories(
  ...
  let ghsaEnrichmentCount = 0;
  let nvdEnrichmentCount = 0;
  let ghsaCapWarned = false;
  let nvdCapWarned = false;
  ...
  const needsNvd = osvResults.some(({ vulns }) =>
    vulns.some((vuln) => (vuln.aliases ?? []).some((alias) => alias.startsWith("CVE-"))),
  );
  if (needsNvd && !config.nvdApiKey) {
    warnings.push({
      code: "NVD_KEY_MISSING",
      message: "NVD CVE enrichment skipped because SAFEFORGE_NPM_NVD_API_KEY is not set. OSV matching still completed.",
    });
  }
  ...
  for (const { node, vulns } of osvResults) {
    for (const vuln of vulns) {
      ...
      if (ghsaAlias && !ghsaCache.has(ghsaAlias)) {
        if (!config.githubToken) {
          ghsaCache.set(ghsaAlias, null);
        } else if (ghsaEnrichmentCount < MAX_GHSA_ENRICHMENTS) {
          ghsaEnrichmentCount += 1;
          ...
          if (!ghsaCapWarned) {
            ghsaCapWarned = true;
            warnings.push({
              code: "GHSA_ENRICHMENT_LIMIT",
              message: `GHSA enrichment capped at ${MAX_GHSA_ENRICHMENTS} advisories to keep scans responsive. OSV matching still completed.`,
            });
          }
        }
      }
      if (cveAlias && !nvdCache.has(cveAlias)) {
        if (!config.nvdApiKey) {
          nvdCache.set(cveAlias, null);
        } else if (nvdEnrichmentCount < MAX_NVD_ENRICHMENTS) {
          nvdEnrichmentCount += 1;
          ...
          if (!nvdCapWarned) {
            nvdCapWarned = true;
            warnings.push({
              code: "NVD_ENRICHMENT_LIMIT",
              message: `NVD enrichment capped at ${MAX_NVD_ENRICHMENTS} CVEs to keep scans responsive. OSV matching still completed.`,
            });
          }
        }
      }
