...
for (const proto of ['net', 'dns']) {
  try {
    const mod = require(proto);
    if (proto === 'net' && typeof mod.connect === 'function') {
      const _origConnect = mod.connect;
      mod.connect = function(...args) {
        const first = args[0];
        const host = typeof first === 'object' ? (first.host || first.hostname || 'localhost') : String(args[1] || 'localhost');
        const port = typeof first === 'object' ? first.port : first;
        _log.push({ type: 'network', method: 'CONNECT', url: 'tcp://' + host + ':' + String(port || '') });
        return _origConnect.apply(this, args);
      };
    }
    if (proto === 'dns') {
      for (const method of ['lookup', 'resolve', 'resolve4', 'resolve6', 'resolveTxt']) {
        if (typeof mod[method] === 'function') {
          const _origDns = mod[method];
          mod[method] = function(hostname, ...args) {
            _log.push({ type: 'network', method: 'DNS', url: 'dns://' + String(hostname) });
            return _origDns.call(this, hostname, ...args);
          };
        }
      }
    }
...
global.Function = function(...args) {
  ...
  _log.push({ type: 'eval', code: 'Function(' + String(body).slice(0, 200) + ')' });
...
process.on('exit', () => {
  try {
    process.stdout.write('\n__SAFEFORGE_NPM_TRACE__' + JSON.stringify(_log) + '__SAFEFORGE_NPM_TRACE_END__\n');
  } catch {}
});
`;
