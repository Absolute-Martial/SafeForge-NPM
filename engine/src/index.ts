...
function enqueueAudit(packageName: string, version: string | undefined, options: AuditRunOptions): Promise<any> {
  return new Promise((resolve, reject) => {
    auditQueue.push({ packageName, version, options, resolve, reject });
    processQueue();
  });
}
...
app.post("/audit/stream", async (c) => {
  ...
  console.log(`[api] starting streaming audit for ${parsed.data.packageName}${parsed.data.version ? `@${parsed.data.version}` : ""}`);
  console.log("[api] scan options:", JSON.stringify(sanitizedOptions));

  // Run audit in background — don't await
  runAudit(parsed.data.packageName, parsed.data.version, options, emit, session.auditId)
...
console.log(`SafeForge NPM Engine starting on ${config.apiHost}:${config.apiPort}`);
