          ...
          <div style={{ borderTop: "1px solid var(--border)", padding: "16px 20px 20px" }}>
            ...
            {advisories.length > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {advisories.slice(0, 50).map((advisory) => (
                ...
                {advisories.length > 50 && (
                  <div style={{ fontSize: "0.74rem", color: "var(--text-muted)", lineHeight: 1.6 }}>
                    Showing the first 50 advisory cards to keep the dashboard responsive. The full advisory list is still included in the JSON report.
                  </div>
                )}
              </div>
            )}
        ...
        {cliBehavior && (
          <div style={{ borderTop: "1px solid var(--border)", padding: "16px 20px 20px" }}>
            ...
            >
              ...
              >
                Node {cliBehavior.nodeVersions.join(", ")}
              </span>
            </div>
            ...
            <SandboxTelemetrySummary results={cliBehavior.results} commandCount={cliBehavior.commandsDiscovered.length} />

            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {cliBehavior.results.length > 0 ? (
                ...
                <div style={{ fontSize: "0.8rem", color: "var(--text-muted)", lineHeight: 1.6 }}>
                  {cliBehavior.commandsDiscovered.length === 0
                    ? "No package bin commands were discovered, so there were no CLI scenarios to execute."
                    : "CLI commands were discovered and the sandbox is preparing results."}
                </div>
              )}
            </div>
          </div>
...
function SandboxTelemetrySummary({ results, commandCount }: { results: CliCommandResult[]; commandCount: number }) {
  const totals = results.reduce(
    (acc, result) => {
      acc.network += result.trace.networkCalls.length;
      acc.dns += result.trace.networkCalls.filter((call) => call.method === "DNS" || call.url.startsWith("dns://")).length;
      acc.process += result.trace.processSpawns.length;
      acc.env += result.trace.envAccess.length;
      acc.fs += result.trace.fsOperations.length;
      acc.eval += result.trace.evalCalls.length;
      return acc;
    },
    { network: 0, dns: 0, process: 0, env: 0, fs: 0, eval: 0 },
  );

  const items = [
    { label: "Commands", value: commandCount },
    { label: "Runs", value: results.length },
    { label: "Web/API", value: totals.network },
    { label: "DNS", value: totals.dns },
    { label: "Processes", value: totals.process },
    { label: "Env reads", value: totals.env },
    { label: "File ops", value: totals.fs },
    { label: "Eval", value: totals.eval },
  ];
  ...
  return (
    ...
    >
      {items.map((item) => {
        const risky = ["Web/API", "DNS", "Processes", "Env reads", "Eval"].includes(item.label) && item.value > 0;
        return (
          <div
            key={item.label}
            style={{
              ...
              borderRadius: "var(--radius-sm)",
              padding: "8px 10px",
              background: risky ? "var(--danger-bg)" : "var(--bg-secondary)",
            }}
          >
            <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.62rem", color: "var(--text-muted)" }}>
              {item.label}
            </div>
            <div style={{ marginTop: 4, fontSize: "1rem", fontWeight: 700, color: risky ? "var(--danger)" : "var(--text)" }}>
              {item.value}
            </div>
          </div>
        );
...
function CliBehaviorCard({ result }: { result: CliCommandResult }) {
  ...
  return (
    ...
    >
      ...
      <RuntimeTraceDetails result={result} riskColor={riskColor} />
    </div>
  );
}

function RuntimeTraceDetails({ result, riskColor }: { result: CliCommandResult; riskColor: string }) {
  const traceItems: Array<{ label: string; value: string; risky?: boolean }> = [];

  for (const call of result.trace.networkCalls.slice(0, 4)) {
    traceItems.push({
      label: call.method === "DNS" || call.url.startsWith("dns://") ? "DNS/Web request" : "Web/API request",
      value: `${call.method} ${call.url}${call.bodyPreview ? ` · body ${call.bodyPreview}` : ""}`,
      risky: true,
    });
  }

  for (const spawn of result.trace.processSpawns.slice(0, 3)) {
    traceItems.push({
      label: "Command execution",
      value: `${spawn.cmd}${spawn.args.length > 0 ? ` ${spawn.args.join(" ")}` : ""}`,
      risky: true,
    });
  }

  for (const envName of result.trace.envAccess.slice(0, 4)) {
    traceItems.push({
      label: "Environment read",
      value: envName,
      risky: /TOKEN|SECRET|PASSWORD|API_KEY|AUTH|NPM_|GITHUB_|OPENAI_/i.test(envName),
    });
  }

  for (const op of result.trace.fsOperations.slice(0, 4)) {
    traceItems.push({
      label: "File access",
      value: `${op.op} ${op.path}${op.preview ? ` · ${op.preview}` : ""}`,
    });
  }

  for (const evalCall of result.trace.evalCalls.slice(0, 2)) {
    traceItems.push({
      label: "Dynamic code",
      value: evalCall.code.slice(0, 160),
      risky: true,
    });
  }
  ...
  if (traceItems.length === 0) {
    return (
      <div style={{ marginTop: 8, fontSize: "0.72rem", color: "var(--text-muted)", fontFamily: "var(--font-mono)" }}>
        Trace: no web/API, DNS, process, env, filesystem, or eval events observed for this run.
      </div>
    );
  }
      ...
      {traceItems.map((item, index) => (
        <div
          ...
          style={{
            ...
            paddingTop: index === 0 ? 7 : 0,
          }}
        >
          <span style={{ fontFamily: "var(--font-mono)", color: item.risky ? riskColor : "var(--text-muted)" }}>
            {item.label}
          </span>
