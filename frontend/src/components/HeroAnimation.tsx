...
function Typed({
  ...
  }, [started, chars, text, speed, onDone]);

  if (!started) return null;
  const finished = chars.length === text.length;
...
/* ────────────────────────────────────────────────────────────────────────── */
/*  SCENARIO 1: Without SafeForge NPM — blind install, compromised             */
/* ────────────────────────────────────────────────────────────────────────── */
...
/* ────────────────────────────────────────────────────────────────────────── */
/*  SCENARIO 2: With SafeForge NPM — catches phishing before install            */
/* ────────────────────────────────────────────────────────────────────────── */
...
function ScenarioWith() {
  ...
  return (
    <TerminalChrome title="terminal" className={phase >= 4 ? "terminal-safe-border" : ""}>
      ...
      <Typed
        text="safeforge-npm install event-stream@3.3.6"
        delay={400}
        speed={30}
        onDone={() => goPhase(1)}
      />
      ...
      {phase >= 1 && (
        <Line delay={200}>
          <span style={{ color: "var(--text-dim)" }}>
            ...
            preparing isolated audit workspace...
          </span>
        </Line>
      )}
      {phase >= 1 && (
        <Line delay={700}>
          <span style={{ color: "var(--text-muted)" }}>
            {">"} resolving package metadata and dependency graph...
          </span>
        </Line>
      )}
      {phase >= 1 && (
        <Line delay={1200} onShow={() => goPhase(2)}>
          <span style={{ color: "var(--accent)" }}>
            {">"} sandbox ready. starting behavior scan.
          </span>
        </Line>
      )}
        ...
        <Line delay={2500} onShow={() => goPhase(4)}>
          <div className="terminal-verdict terminal-verdict-danger" style={{ marginTop: 8 }}>
            ...
            <div className="verdict-action" style={{ marginTop: 6 }}>
              {">"} verdict ready before installation
            </div>
          </div>
        </Line>
      )}
      ...
      {phase >= 4 && (
        <Line delay={1500}>
          <span style={{ color: "var(--safe)", fontWeight: 700, display: "block", marginTop: 4 }}>
            <ShieldIcon /> Crisis averted. SafeForge NPM caught it before a single byte ran on your machine.
          </span>
        </Line>
      )}
    </TerminalChrome>
...
export function HeroAnimation() {
  ...
  const switchScene = (nextScene: "without" | "with") => {
    setScene(nextScene);
    setRunId((id) => id + 1);
  };
  ...
  return (
    <div className="hero-animation-container">
      <div className="hero-scene-labels">
        ...
        >
          without SafeForge NPM
        </button>
        <button
          className="scene-tab"
          onClick={() => switchScene("with")}
          style={{
            color:
      ...
      <div className="hero-terminal-wrapper">
        {scene === "without" ? (
          <ScenarioWithout
            key={`without-${runId}`}
            onComplete={() => switchScene("with")}
          />
        ) : (
          <ScenarioWith key={`with-${runId}`} />
        )}
      </div>
    </div>
  );
